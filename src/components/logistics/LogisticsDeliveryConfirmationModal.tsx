...
export const LogisticsDeliveryConfirmationModal: React.FC<DeliveryModalProps> = ({ shipment, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 max-w-lg w-full border border-gray-200 dark:border-gray-800 shadow-2xl animate-in fade-in zoom-in duration-200 space-y-4">
        {/* Header */}
        ...
        <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-xs space-y-2">
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 font-black text-sm text-blue-600"
              required
            />
          </div>

          {qtyDiff !== 0 && (
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/60 border border-amber-300 text-amber-900 dark:text-amber-200 space-y-1">
        ...
        <div className="space-y-3 text-xs">
          <div>
            <label className="block font-bold text-gray-700 dark:text-gray-300 mb-1">Receiver Name / Signature Contact</label>
            <input
              type="text"
              value={receiverName}
              onChange={(e) => setReceiverName(e.target.value)}
              className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 font-bold"
              required
            />
          </div>

          <div>
            <label className="block font-bold text-gray-700 dark:text-gray-300 mb-1">Condition Notes</label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              rows={2}
              className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 font-semibold"
            />
          </div>
        </div>
        ...
        <form onSubmit={handleConfirmSubmit} className="space-y-3 pt-2">
          <label className="p-3.5 rounded-xl border-2 border-blue-500 bg-blue-50/60 dark:bg-blue-950/40 flex items-center gap-3 cursor-pointer text-xs font-extrabold text-blue-900 dark:text-blue-200">
            <input
              type="checkbox"
