...
export const AdminRating: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-b from-purple-500/10 via-indigo-500/5 to-white dark:to-gray-800 border-2 border-purple-500 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-extrabold px-3 py-1 rounded-full bg-purple-600 text-white uppercase tracking-wider shadow-xs">
            Current Agent Score
          </span>
          <h3 className="text-4xl font-black text-gray-900 dark:text-white mt-2 flex items-center gap-2">
            {rating.overallRating.toFixed(1)} / 5.0
            <span className="flex text-purple-500">
              <Star className="w-8 h-8 fill-purple-500" />
            </span>
          </h3>
          <p className="text-xs text-purple-700 dark:text-purple-300 font-bold mt-1 flex items-center gap-1">
        ...
        <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 text-xs text-amber-900 dark:text-amber-200 max-w-sm space-y-1">
          <span className="font-extrabold flex items-center gap-1 text-amber-800 dark:text-amber-300">
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-gray-700/50 space-y-1">
            ...
            <strong className="text-base text-gray-900 dark:text-white block">{rating.breakdown.taskCompletion} / 5.0</strong>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-md space-y-3">
        ...
        <div className="space-y-2 text-xs">
          {rating.recentIssues.map((iss, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300">
              • {iss}
            </div>
          ))}
        </div>
