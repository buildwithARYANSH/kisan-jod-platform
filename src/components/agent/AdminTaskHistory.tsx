              ...
              {tasks.map((task) => (
                <tr key={task.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="p-3 font-mono">
                    <strong className="text-gray-900 dark:text-white block">{task.id}</strong>
                    <span className="text-[10px] text-gray-400">{task.pickupDate}</span>
                  </td>
                  <td className="p-3 font-bold text-gray-900 dark:text-white">{task.cropName} ({task.quantity.toLocaleString()} {task.unit})</td>
