...
export const CompanyOrderTracking: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {(!orders || orders.length === 0 || !activeOrder) ? (
        <div className="bg-white p-12 rounded-3xl border border-slate-200 shadow-md text-center">
          <div className="w-16 h-16 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mx-auto mb-4">
            <Truck className="w-8 h-8" />
          </div>
          <h3 className="text-lg font-black text-slate-900 font-fraunces">
            No Active Orders Being Tracked
          </h3>
          <p className="text-xs sm:text-sm text-slate-500 font-semibold max-w-md mx-auto mt-1">
            There are currently no active shipments registered for your company. Once you submit a demand and match produce with local farmers, live shipment tracking and driver details will appear here.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-md space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg sm:text-xl font-black text-slate-900 font-fraunces">
                  {activeOrder.cropName} ({activeOrder.quantity?.toLocaleString() || 0} {activeOrder.unit || 'kg'})
                </h3>
              ...
              <span className="px-3.5 py-1 rounded-full bg-blue-100 text-blue-950 text-xs font-black border border-blue-300">
                Status: {activeOrder.currentStage}
              </span>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-2.5">
                {(activeOrder.stages || []).map((stg, idx) => (
                  <div
                    key={stg.name}
            ...
            <div className="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm font-semibold">
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                ...
                <p className="font-black text-slate-900 text-base font-fraunces">
                  {activeOrder.logisticsPartner || 'Express Agri Transport Ltd'}
                </p>
                <div className="space-y-1.5 text-slate-700 font-semibold">
                  <p className="flex items-center gap-2">
                    <User className="w-4 h-4 text-blue-600" />
                    Driver: <strong className="text-slate-900 font-extrabold">{activeOrder.driverName || 'Rajesh Kumar'}</strong>
                  </p>
                  <p className="flex items-center gap-2">
                    <Truck className="w-4 h-4 text-blue-600" />
                    Vehicle: <strong className="text-slate-900 font-extrabold">{activeOrder.vehicleNumber || 'PB-10-CZ-4491'} ({activeOrder.vehicleType || 'Refrigerated Container'})</strong>
                  </p>
                  <p className="flex items-center gap-2">
                    <Phone className="w-4 h-4 text-blue-600" />
                    Driver Contact: <strong className="text-slate-900 font-extrabold">{activeOrder.contact || '+91 98721 99881'}</strong>
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 space-y-2 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-black text-blue-950 mt-1 font-fraunces">
                    {activeOrder.estimatedArrival || 'Tomorrow, 02:00 PM'}
                  </p>
