...
import { subscribeToSyncEvents } from '../services/unifiedSync';
...
interface AdminContextType {
  ...
  offlinePendingSyncCount: number;
...
export const AdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [isOfflineMode, setIsOfflineMode] = useState<boolean>(() => {
    try { return localStorage.getItem('kisan_agent_offline') === 'true'; } catch { return false; }
  });
  const [offlinePendingSyncCount, setOfflinePendingSyncCount] = useState<number>(2);

  const toggleOfflineMode = () => {
    setIsOfflineMode((prev) => {
      const next = !prev;
      try { localStorage.setItem('kisan_agent_offline', String(next)); } catch (e) { console.warn(e); }
      return next;
    });
  };

  const syncOfflineData = () => {
    setOfflinePendingSyncCount(0);
  ...
  // Reactive Cross-Port Listener (Port 5173 -> Port 5174 Task Auto-Assignment)
  useEffect(() => {
    const unsubscribe = subscribeToSyncEvents((payload) => {
      if (payload.type === 'CROP_ADDED') {
        try {
