    ...
    <div className="space-y-6">
      ...
      <div className="bg-white  p-5 rounded-2xl border border-gray-200  shadow-md">
        ...
        <div className="space-y-3 text-xs">
          {issues.map((iss) => (
            <div key={iss.id} className="p-4 rounded-xl bg-gray-50  border border-gray-200  space-y-1">
              <div className="flex justify-between items-center">
                <span className="font-bold text-gray-900  text-sm">{iss.issueType}</span>
                <span className="px-2.5 py-0.5 rounded-full bg-amber-100  text-amber-800  text-[10px] font-bold">
                  {iss.status}
                </span>
              </div>
              <p className="text-gray-500 font-mono text-[11px]">{iss.relatedTaskOrOrder} • {iss.reportedDate}</p>
              <p className="text-gray-700  pt-1">{iss.description}</p>
            </div>
          ))}
        </div>
      </div>
