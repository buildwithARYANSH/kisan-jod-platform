            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {vehicles.map((v) => (
                <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="p-3">
                    <strong className="text-gray-900  font-mono text-sm block">{v.registrationNumber}</strong>
                    <span className="text-[10px] font-mono text-gray-400">{v.id}</span>
                  </td>
                  <td className="p-3 font-semibold">{v.vehicleType}</td>
                  <td className="p-3 font-black text-blue-600 ">{v.capacityKg.toLocaleString()} kg</td>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70  flex items-center justify-center p-4">
          <div className="bg-white  rounded-3xl p-6 max-w-md w-full border border-gray-200  shadow-2xl animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-bold text-gray-900  mb-3">Add Vehicle to Fleet</h3>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-3 text-xs sm:text-sm">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 rounded-xl border bg-gray-50  font-bold"
                  required
                />
              </div>

              <div>
                <label className="block font-bold text-gray-700  mb-1">Vehicle Type Specification</label>
                <select
                  value={vType}
              ...
              <div>
                <label className="block font-bold text-gray-700  mb-1">Max Payload Capacity (kg)</label>
                <input
                  type="number"
                  value={capacity}
                  onChange={(e) => setCapacity(Number(e.target.value))}
                  className="w-full p-2.5 rounded-xl border bg-gray-50  font-bold text-blue-600"
                  required
                />
              </div>
